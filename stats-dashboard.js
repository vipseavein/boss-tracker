...
recent:["Có thao tác <24h","Activity <24h","May aksyon <24h","Atividade <24h"],
warn:["Cần kiểm tra 24–48h","Review 24–48h","Suriin 24–48h","Verificar 24–48h"],
danger:["Cần kiểm tra ≥48h","Review ≥48h","Suriin ≥48h","Verificar ≥48h"],
unknown:["Thiếu lịch sử","Missing history","Kulang na kasaysayan","Histórico insuficiente"],
off:["Không hoạt động ≥24h","Inactive ≥24h","Hindi aktibo ≥24h","Inativo ≥24h"],
...
function status(c,now){return c.unknown||!c.last?"unknown":c.suspicious?(now-c.anchor>=172800000?"danger":"warn"):c.stale?"off":"recent"}
function bars(users){
...
function html(r,now,days){
 ...
 const coverage=r.bossCounts.map(b=>{const bad=r.suspicious.filter(c=>c.boss===b.name).length;return '<div class="d-rank"><div class="d-name">'+e(b.name)+'</div><strong>'+b.active+'/30</strong><div class="d-track"><i style="width:'+(b.active-bad)/30*100+'%"></i><i class="d-red" style="width:'+bad/30*100+'%"></i></div></div>'}).join("");
 const warnings=[...r.suspicious].sort((a,b)=>a.anchor-b.anchor).slice(0,6).map(c=>'<button class="d-warning" data-cell="'+e(c.id)+'"><span>'+e(c.boss)+' · '+c.ch+'</span><strong>'+Math.floor((now-c.anchor)/3600000)+' '+t("hours")+'</strong></button>').join("")||'<p>'+t("empty")+'</p>';
 return '<div class="d-dashboard" data-raw><section class="panel d-map"><h2>'+t("map")+'</h2><p>'+t("hint")+'</p><div class="d-legend">'+["recent","warn","danger","unknown","off"].map(k=>'<span><i class="'+k+'"></i>'+t(k)+'</span>').join("")+'</div><div class="d-mapscroll">'+map+'</div></section><div class="d-layout"><section class="panel"><h2>'+t("activity")+'</h2><div class="d-legend"><span><i class="recent"></i>'+t("on")+'</span><span><i class="danger"></i>'+t("stop")+'</span></div><div class="d-chart">'+chart+'</div><p class="d-note">'+t("note")+'</p></section><section class="panel"><h2>'+t("alert")+' <span class="badge">'+r.suspicious.length+'</span></h2>'+warnings+'</section><section class="panel"><h2>'+t("coverage")+'</h2><p class="d-note">'+t("danger")+' / '+t("warn")+': <span class="d-dot"></span></p><div class="d-scroll">'+coverage+'</div></section><section class="panel"><div class="d-rankhead"><h2>'+t("rank")+'</h2><select id="rankOrder"><option value="most" '+(order==="most"?"selected":"")+'>'+t("most")+'</option><option value="least" '+(order==="least"?"selected":"")+'>'+t("least")+'</option></select></div><div id="rankBars">'+bars(r.users)+'</div></section></div></div>';
}
function bind(){
...
window.BTDashboard={html,bind,t};
})();
